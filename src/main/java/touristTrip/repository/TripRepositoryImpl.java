...
public class TripRepositoryImpl {
    ...
    public List<AvgPrice> avgPriceList() {
        TypedQuery<AvgPrice> query = entityManager.createQuery("SELECT new touristTrip.object.AvgPrice(t.id, t.destination, Round(avg(ct.price),2) ) FROM Trip t " +
    ...
    public List<SumPrice> getSum() {
        TypedQuery<SumPrice> query = entityManager.createQuery("SELECT new touristTrip.object.SumPrice (count(t.id), t.destination, Round(sum(ct.price),2) ) FROM Trip t " +
